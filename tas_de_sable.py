    ...
    for i in range(4):
        for j in range(4):
            ...
            if mat[4*i+j]==0:
                ...
                taquin.create_text(C, text=mat[4*i+j], fill="white",font=FONT)
a = str(input("Voulez-vous continuer la dernière partie? (O/N) "))
if a=="O":
    #on ouvre le fichier texte
    fichier = open("dernierepartie.txt", "r")
    #sur la premiere ligne est ecrite la grille
    f =fichier.readlines()[0].split(",")
    fichier.close()
    mat = [int(i) for i in f]
else:
    #on mélange la grille en reproduisant des mouvements aléatoires, un grand nombre de fois car certaines configurations ne sont pas résolubles
